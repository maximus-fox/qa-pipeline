...
4. **Emit follow-up work**: for each gap, a concrete task addressed to the right role (screen, environment, what to prove). **Name ONLY the roles that actually have follow-up work** — the engine re-runs just those, not the whole fleet. Map gaps (a screen/route missing from `qa-map.md`) are addressed to `recon` — the engine runs a targeted map update before the follow-up round; naming a role with no work is itself a gate error.
